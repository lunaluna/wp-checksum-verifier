...
このエンドポイントは冪等: 既にrunが進行中なら新規runを作らずそのrun_idを
返す。Action Scheduler が利用可能なら、設定した時間予算(設定画面。常に
サーバーの `max_execution_time` の70%を上限にクランプ)内でキューの処理も
オポチュニスティックに行う.

## 設定

設定画面(マルチサイトではネットワーク管理画面)から、WP-Cronの日次実行時刻
(UTC)・REST時間予算・RESTトークンの発行を設定できる.
